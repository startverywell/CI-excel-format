    ...
    <div class="row justify-content-center mt-0">
        <div class="col-12 col-sm-12 col-md-12 col-lg-12 text-center p-0 mt-3 mb-2">
            <div class="card px-0 pt-4 pb-0 mt-3 mb-3">
                <h2><strong>REVIEW CONTAINERS</strong></h2>
                ...
                <div class="row">
                    <div class="col-md-12 mx-0">
                        <form id="msform" action="<?php echo site_url('billcheck/checkall');?>" method="POST">
                            ...
                            <fieldset>
                                <div class="form-card">
                                    <h2 class="fs-title mb-2">REVIEW CONTAINERS</h2>
                                            ...
                                            <?php 
                                                ...
                                                if ($details != false) {
                                                    foreach ($details as $detali) {
                                                        ...
                                                        if($detali->single_top == 1 || $detali->multi_top == 1){
                                                            $sku = $detali->notes ?? '';
                                                            $qty = $detali->ctn ?? '';
                                                        } else if($detali->asst == 1) {
                                                            $sku = $detali->style ?? '';
                                                            $qty = $detali->ctn ?? '';
                                                        }
                                                        else {
                                                        ...
                                                        echo '<tr>';
                                                        echo '<td>'.$no++.'</td>';
                                                        echo '<td>'.$header->shipment_name.'</td>';
                                                        echo '<td>'.$detali->container_name.'</td>';
                                                        echo '<td>'.$detali->po.'</td>';
                                                        echo '<td>'.$sku.'</td>';
                                                        echo '<td>'.$detali->cbm.'</td>';
                                                        echo '<td>'.$qty.'</td>';
